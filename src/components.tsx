...
export function ChartCard({title,series,kind='bar',wide=false,stacked=false,actions,caption}:{title:string;series?:Series;kind?:'bar'|'line';wide?:boolean;stacked?:boolean;actions?:ReactNode;caption?:string}){const empty=!series?.labels?.length;const dual=series?.datasets?.some(d=>d.yAxisID==='y1');const chartOpts=stacked?stackedOpts:dual?dualOpts:opts;return <section className={`card chart-card ${wide?'wide':''}`}><div className="chart-card-head"><h3>{title}</h3>{actions}</div>{caption&&<p className="detail-caption">{caption}</p>}<div className="chart">{empty?<div className="empty">No data returned</div>:kind==='line'?<Line data={series!} options={chartOpts}/>:<Bar data={series!} options={chartOpts}/>}</div></section>}
